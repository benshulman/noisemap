...
# target in
parks = pd.read_csv('/Users/Ben/Dropbox/Insight/data-merge/merge-aws/coord-in/parks_clean.csv')
...
# calculate distance
def dist_calc(parkname, lat, lng):
    '''
    ...
    '''
    id_dict = {'parkname': parkname, 'lat': lat, 'lng': lng}
...
def main():
    with futures.ProcessPoolExecutor() as executor:
        results = list(
            tqdm.tqdm(
                executor.map(dist_calc, parks.name, parks.lat, parks.lng),
                total = len(parks)
                )
            )
    test_df = pd.DataFrame(results)
    test_df.to_csv('./out/parks-merge.csv')
    end = time.time()
