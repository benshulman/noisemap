...
# target in
grid = pd.read_csv('./coord-in/grid.csv')
...
def dist_calc(lat, lng):
    ...
    dict_out = {**point_dict, **airport_dict, **line_dict}
    return dict_out
...
def main():
    ...
    with futures.ProcessPoolExecutor() as executor:
        results = list(
            tqdm.tqdm(
                executor.map(dist_calc,
                    grid.lat[:10000],
                    grid.lng[:10000]),
                total = len(grid[:10000])
                )
            )
    test_df = pd.DataFrame(results)
    test_df.to_csv('./out/grid-merge1.csv')
    end = time.time()
    ...
    with futures.ProcessPoolExecutor() as executor:
        results = list(
            tqdm.tqdm(
                executor.map(dist_calc,
                    grid.lat[10000:20000],
                    grid.lng[10000:20000]),
                total = len(grid[10000:20000])
                )
            )
    test_df = pd.DataFrame(results)
    test_df.to_csv('./out/grid-merge2.csv')
    end = time.time()
    print("Took %f seconds..." %(end - start))
    # # took 22.201501 seconds
    # block 3 -------------------------------------
    with futures.ProcessPoolExecutor() as executor:
        results = list(
            tqdm.tqdm(
                executor.map(dist_calc,
                    grid.lat[20000:30000],
                    grid.lng[20000:30000]),
                total = len(grid[20000:30000])
                )
            )
    test_df = pd.DataFrame(results)
    test_df.to_csv('./out/grid-merge3.csv')
    end = time.time()
    print("Took %f seconds..." %(end - start))
    # block 4 -------------------------------------
    with futures.ProcessPoolExecutor() as executor:
        results = list(
            tqdm.tqdm(
                executor.map(dist_calc,
                    grid.lat[30000:],
                    grid.lng[30000:]),
                total = len(grid[30000:])
                )
            )
    test_df = pd.DataFrame(results)
    test_df.to_csv('./out/grid-merge4.csv')
    end = time.time()
    print("Took %f seconds..." %(end - start))
